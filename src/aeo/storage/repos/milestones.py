...
def claim_pack_owner(client_id: int, user_id: str) -> str | None:
    """Claim this client's UNOWNED pack milestones for ``user_id``; return the owner that
    stands after the attempt (an existing owner is never displaced).

    The counterpart to :func:`pack_owner_of`, and the reason P5's gate was inert: a board
    generated by the PIPELINE has no user in scope, so it was born unowned and nothing ever
    stamped it afterwards — leaving ``_require_ticket_owner`` permanently in its
    "unowned → allow" branch for every board in production.

    Two properties matter:

    * **All-or-nothing per client.** The ``NOT EXISTS`` guard means a client whose pack rows
      are already partly owned is left completely alone. Without it, a stranger reading a
      pack that happened to be generated in a later re-gen would stamp *their* id onto the
      rows the real owner's stamp had not reached, splitting one board across two owners —
      and ``pack_owner_of`` returns whichever row it finds first, so the gate would then
      admit or refuse the true owner at random.
    * **Atomic.** The claim is one UPDATE whose WHERE clause carries the whole precondition,
      so two first-viewers racing on the same board cannot both win; the loser's UPDATE
      matches zero rows and the following SELECT tells it who actually owns the board.
    """
    with transaction() as conn, conn.cursor() as cur:
        cur.execute(
            """
            UPDATE implementation_milestones m
             ...
             WHERE m.client_id = %s
               ...
               AND NOT EXISTS (
                     SELECT 1 FROM implementation_milestones o
                      WHERE o.client_id = m.client_id
                        AND o.milestone_key LIKE 'pack:%%'
