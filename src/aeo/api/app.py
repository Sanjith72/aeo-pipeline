...
def _claim_ticket_board(user: User | None, client_id: int) -> None:
    """Stamp ``owner_user_id`` on an UNOWNED ticket board for the signed-in viewer.

    The ownership rule, stated once (P5): **the first authenticated user to touch a run's
    pack milestones owns them.** In practice a board is owned from birth — ``POST /api/audit``
    now carries the caller's id down to ``generate_tickets_from_run`` — and this is the
    fallback for the two cases that stamp cannot cover: boards generated before this shipped,
    and boards generated by a genuinely anonymous audit. Anonymous boards stay unowned and
    fully open, which is what the signed-out free tier depends on.

    Called from the READ paths on purpose. It has to be: the mutation routes already refuse
    based on ownership, so claiming only there would mean the very first mutation on an
    unowned board is the one that decides the owner — indistinguishable from today's
    everyone-passes behaviour for as long as nobody mutates.

    Tradeoff worth naming: on a board created by an ANONYMOUS audit, whichever signed-in user
    reads it first becomes the owner, and Pack 1 is readable by everyone. Someone who knew a
    stranger's run id could therefore claim that stranger's anonymous board. That is strictly
    better than the status quo (where every logged-in user can mutate every board on a domain
    they hold any entitlement for), and it cannot touch a board whose audit was started while
    signed in — which is every board a paying customer has.

    Best-effort: a read must never 500 because a claim lost a race or hit a missing
    ``app_users`` row.
    """
    if user is None:
        return
    ...
    try:
        # Read before writing: these are READ routes, and an unconditional UPDATE would put
        # a WAL write on every ticket fetch for the entire life of an owned board. The
        # SELECT is index-backed and the claim's own WHERE clause still settles any race,
        # so this is purely about not writing on the overwhelmingly common path.
        if milestones_repo.pack_owner_of(client_id) is None:
            milestones_repo.claim_pack_owner(client_id, str(user.id))
    except Exception as exc:  # claiming is bookkeeping — never fail the read over it
        from ..logging import get_logger

        get_logger(__name__).warning(
            "pack_owner_claim_failed", client_id=client_id, error=str(exc)
        )
...
def _require_ticket_owner(user: User | None, run_id: int, client_id: int) -> None:
    ...
    A signed-in caller CLAIMS an unowned board here rather than being waved through it, so
    the first mutation settles ownership instead of leaving the board open forever. Without
    that, the "unowned" branch stayed reachable for the entire life of every board the
    pipeline generated — which is what made this gate inert in production.
    """
    from ..storage.repos import entitlements as entitlements_repo
    from ..storage.repos import milestones as milestones_repo
    from ..storage.repos import runs as runs_repo
    ...
    if owner is None:
        _claim_ticket_board(user, client_id)
        return
    if user is not None:
        if user.id == owner:
            return
...
def get_tickets(run_id: int, user: User | None = Depends(get_optional_user)) -> dict[str, Any]:
    ...
    **Gated (v5 CH-02a):** the response is FILTERED to the viewer's unlocked packs. A ticket
    ...
    if target is not None:
        _claim_ticket_board(user, target.id)
...
@app.post("/api/audit")
def start_audit(
    req: AuditRequest, user: User | None = Depends(get_optional_user)
) -> dict[str, Any]:
    ...
    The caller's verified id (P5) rides down to ticket generation so the resulting board is
    OWNED from birth rather than born anonymous and claimed later — the run's initiator is
    the least surprising owner, and it closes the window in which another signed-in user
    could read the board first and claim it. Anonymous callers are unchanged: ``None``
    propagates and the board stays unowned and open, which the free tier depends on.

    Note the dedupe below: a second caller joining an in-flight audit does NOT become the
    owner. Ownership follows whoever actually started the run."""
    domain = req.domain.strip()
    if not domain:
        raise HTTPException(status_code=422, detail="domain is required")
