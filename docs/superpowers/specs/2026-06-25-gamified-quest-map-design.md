...
- `TaskDetailPanel.tsx` — focus-trapped dialog: title, why, actions, Mark Complete / Verify; reuses the shared `TaskGuide`.
- `web/components/tracker/TaskGuide.tsx` — the how-to / DIY / Developer-handoff panel **extracted** from `MilestoneDashboard.tsx` so both views share one implementation.
